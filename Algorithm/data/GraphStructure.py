...
class Data_Structure():
    ...
    def get_alpha_carbon_chain(self):
        ...
        for chain in self._structure.get_chains():
            ch_alpha_C = []



            for residue in chain.get_list():
                #print("residute = ", residue.__repr__(), residue.get_resname())
    ...
    def check_chi(self, chain):
        # проверка на угол chi
        chain.atom_to_internal_coordinates(verbose=True)
        print(chain)
        r2 = chain.child_list[0]
        print("r2", r2)
        r2ic = r2.internal_coord
        # print(r2ic)
        r2chi1_object = r2ic.pick_angle("chi1")
        r2chi1_key = r2chi1_object.atomkeys
        print("r2chi1_key", r2chi1_key)

    def calculate_neighbors(self, alphaSkeleton):
        #вычисляем ближайшие атмоы в радиусе
        arr= []
        i=0
        for atom in alphaSkeleton['A']:
           if(i<5):
                arr.append(atom)
                i+=1
        ...
        #for atom in arr:
            #atoms = pdb.Selection.unfold_entities(arr, 'A')
        ns = pdb.NeighborSearch(arr)

        close_atoms = ns.search(arr[1].coord, 5)    #радиус 5
        print(close_atoms)
